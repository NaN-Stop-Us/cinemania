import{f as F,a as E,B as b,E as $,s as C,I as x,i as I,r as D,b as G,c as L}from"./footer-CRMM4mgP.js";let M=document.querySelector("#weekly-trends-list"),_=document.getElementById("upcoming-card"),S={};(async()=>S=await F())();const H=window.innerWidth;function q(t){return H<=767?t.slice(0,1):t.slice(0,3)}function P(t){const a=t.map(n=>{const{id:r,title:e,poster_path:o,release_date:l,vote_average:c,genre_ids:s}=n,i=l?l.slice(0,4):"N/A",p=o?`${x}/w500${o}`:"https://via.placeholder.com/395x574?text=No+Image",g=(s==null?void 0:s.map(w=>S[w]).join(", "))||"";return`
    ...
    `}).join("");M.innerHTML=a,document.querySelectorAll(".weekly-card").forEach(n=>{const r=parseFloat(n.dataset.rating),e=n.querySelector(".weekly-card__rating");L(r,e)})}async function U(){try{const t=await E(b,$.TRENDING_WEEK),a=q(t.results);P(a)}catch(t){M.innerHTML='<p style="color:red;">Failed to load movies 😢</p>',console.error("Fetch error:",t)}}function W(){const t=new Date,a=t.getFullYear(),n=t.getMonth(),r=new Date(a,n,1).toISOString().split("T")[0],e=new Date(a,n+1,0).toISOString().split("T")[0];return{start:r,end:e}}async function j(){try{const{start:t,end:a}=W(),r=(await E(b,$.UPCOMING_MOVIES)).results.filter(i=>i.release_date>=t&&i.release_date<=a);if(!r.length){_.innerHTML='<p class="upcoming__info">No upcoming movies found this month.</p>';return}const e=r[Math.floor(Math.random()*r.length)],o=e.backdrop_path?`${x}/original${e.backdrop_path}`:"https://via.placeholder.com/800x450",l=e.genre_ids.map(i=>S[i]||"Unknown").join(", "),c=I(e.id);_.innerHTML=`
...
//# sourceMappingURL=main-DpmwlTSg.js.map

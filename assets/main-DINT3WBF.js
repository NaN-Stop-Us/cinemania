...
<stop offset="1" stop-color="#F89F19" stop-opacity="0.68"/>
...
</svg>`,o=e/2,i=Math.floor(o),c=o%1>=.5?1:0,w=5-i-c;t.innerHTML="";const v=l=>{const m=document.createElement("div");m.innerHTML=l.trim();const E=m.firstElementChild;t.appendChild(E)};for(let l=0;l<i;l++)v(a);c&&v(r);for(let l=0;l<w;l++)v(s)}async function G(){try{const e=await b(f,p.POPULAR_MOVIES),t=I(e.results),a=`${L}${p.IMG_W1280}${t.backdrop_path}`;d.style.backgroundImage=`url('${a}')`,d.innerHTML=`
    ...
    `;const r=d.querySelector(".stars");O(t.vote_average,r),d.querySelector(".more-details").addEventListener("click",()=>{$(t)}),d.querySelector(".watch-trailer").addEventListener("click",async()=>{const s=await M(t.id);s?_(s):(n.innerHTML=`
        ...
        `,n.classList.add("active"),n.querySelector(".close-span-btn").addEventListener("click",g),n.querySelector(".overlay").addEventListener("click",g))})}catch(e){console.error("Catalog hero error:",e)}}G();function $(e){var i;const t=document.getElementById("movie-detail-modal"),a=`${L}${p.IMG_W500}${e.poster_path}`,r=((i=e.genre_ids)==null?void 0:i.map(c=>u==null?void 0:u[c]).join(", "))||"N/A";t.innerHTML=`
  ...
  `,t.classList.add("active");const s=c=>{c.key==="Escape"&&o()};document.addEventListener("keydown",s);function o(){t.classList.remove("active"),t.innerHTML="",document.removeEventListener("keydown",s)}t.querySelector(".close-span-btn-details").addEventListener("click",o),t.querySelector(".detail-overlay").addEventListener("click",o)}
//# sourceMappingURL=main-DINT3WBF.js.map

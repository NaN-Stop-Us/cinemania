import{a as b}from"./vendor-DDD7fsZd.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const a of r)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function o(r){const a={};return r.integrity&&(a.integrity=r.integrity),r.referrerPolicy&&(a.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?a.credentials="include":r.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(r){if(r.ep)return;r.ep=!0;const a=o(r);fetch(r.href,a)}})();window.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("theme-toggle"),t=document.body;localStorage.getItem("theme")==="light"&&t.classList.add("light-theme"),e.addEventListener("click",()=>{t.classList.toggle("light-theme");const s=t.classList.contains("light-theme");localStorage.setItem("theme",s?"light":"dark")})});const S="ed2682fe1cd0ecc2efbcdeb7464c5ffd",y="https://api.themoviedb.org/3",E="https://image.tmdb.org/t/p",p={POPULAR_MOVIES:"/movie/popular",UPCOMING_MOVIES:"/movie/upcoming",TRENDING_WEEK:"/trending/movie/week",TRENDING_DAY:"/trending/movie/day",SEARCH_MOVIES:"/search/movie",GENRE_LIST:"/genre/movie/list",MOVIE_DETAILS:e=>`/movie/${e}`,MOVIE_VIDEOS:e=>`/movie/${e}/videos`,IMG_ORIGINAL:"/original",IMG_W500:"/w500",IMG_W780:"/w780",IMG_W1280:"/w1280"};async function w(e,t,o={}){try{return(await b.get(`${e}${t}`,{params:{api_key:S,language:"en-US",page:1,...o}})).data}catch(s){throw console.error("Error fetching data:",s),s}}async function M(){try{const e=await b.get(`${y}${p.GENRE_LIST}`,{params:{api_key:S,language:"en-US"}}),t={};return e.data.genres.forEach(o=>{t[o.id]=o.name}),t}catch(e){return console.error("Error fetching genres:",e),{}}}const f="myLibrary";function _(e){const t=JSON.parse(localStorage.getItem(f))||[];t.some(o=>o.id===e.id)||(t.push(e),localStorage.setItem(f,JSON.stringify(t)))}function O(e){let t=JSON.parse(localStorage.getItem(f))||[];t=t.filter(o=>o.id!==e),localStorage.setItem(f,JSON.stringify(t))}function L(e){return(JSON.parse(localStorage.getItem(f))||[]).some(o=>o.id===e)}let m={};(async()=>m=await M())();const u=document.querySelector("#catalog-hero"),l=document.getElementById("trailer-modal");u?T():console.log("Catalog hero element not found, skipping initialization");function k(e){const t=Math.floor(Math.random()*e.length);return e[t]}async function $(e){try{const o=(await w(y,p.MOVIE_VIDEOS(e))).results.find(s=>s.type==="Trailer"&&s.site==="YouTube");return o?`https://www.youtube.com/embed/${o.key}`:null}catch{return null}}function G(e){l.innerHTML=`
...
<stop offset="1" stop-color="#F89F19" stop-opacity="0.68"/>
...
</svg>`,a=e/2,i=Math.floor(a),c=a%1>=.5?1:0,g=5-i-c;t.innerHTML="";const d=n=>{const h=document.createElement("div");h.innerHTML=n.trim();const I=h.firstElementChild;t.appendChild(I)};for(let n=0;n<i;n++)d(o);c&&d(s);for(let n=0;n<g;n++)d(r)}async function T(){try{const e=await w(y,p.POPULAR_MOVIES),t=k(e.results),o=`${E}${p.IMG_W1280}${t.backdrop_path}`;u.style.backgroundImage=`url('${o}')`,u.innerHTML=`
    ...
    `;const s=u.querySelector(".stars");x(t.vote_average,s),u.querySelector(".more-details").addEventListener("click",()=>{N(t)}),u.querySelector(".watch-trailer").addEventListener("click",async()=>{const r=await $(t.id);r?G(r):(l.innerHTML=`
          <div class="overlay"></div>
        ...
        `,l.classList.add("active"),l.querySelector(".close-span-btn").addEventListener("click",v),l.querySelector(".overlay").addEventListener("click",v))})}catch(e){console.error("Catalog hero error:",e)}}function N(e,t){var d;const o=document.getElementById("movie-detail-modal"),s=`${E}${p.IMG_W500}${e.poster_path}`,r=((d=e.genre_ids)==null?void 0:d.map(n=>m==null?void 0:m[n]).join(", "))||"N/A",a=L(e.id);o.innerHTML=`

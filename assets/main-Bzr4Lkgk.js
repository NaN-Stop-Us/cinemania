import{f as F,a as E,B as b,E as $,s as C,I as x,i as I,r as D,b as G,c as L}from"./footer-jHw531ic.js";let M=document.querySelector("#weekly-trends-list"),_=document.getElementById("upcoming-card"),S={};(async()=>S=await F())();function H(t){return t.slice(0,3)}function q(t){const n=t.map(a=>{const{id:r,title:e,poster_path:o,release_date:l,vote_average:c,genre_ids:s}=a,i=l?l.slice(0,4):"N/A",p=o?`${x}/w500${o}`:"https://via.placeholder.com/395x574?text=No+Image",g=(s==null?void 0:s.map(w=>S[w]).join(", "))||"";return`
    ...
    `}).join("");M.innerHTML=n,document.querySelectorAll(".weekly-card").forEach(a=>{const r=parseFloat(a.dataset.rating),e=a.querySelector(".weekly-card__rating");L(r,e)})}async function P(){try{const t=await E(b,$.TRENDING_WEEK),n=H(t.results);q(n)}catch(t){M.innerHTML='<p style="color:red;">Failed to load movies 😢</p>',console.error("Fetch error:",t)}}function U(){const t=new Date,n=t.getFullYear(),a=t.getMonth(),r=new Date(n,a,1).toISOString().split("T")[0],e=new Date(n,a+1,0).toISOString().split("T")[0];return{start:r,end:e}}async function j(){try{const{start:t,end:n}=U(),r=(await E(b,$.UPCOMING_MOVIES)).results.filter(i=>i.release_date>=t&&i.release_date<=n);if(!r.length){_.innerHTML='<p class="upcoming__info">No upcoming movies found this month.</p>';return}const e=r[Math.floor(Math.random()*r.length)],o=e.backdrop_path?`${x}/original${e.backdrop_path}`:"https://via.placeholder.com/800x450",l=e.genre_ids.map(i=>S[i]||"Unknown").join(", "),c=I(e.id);_.innerHTML=`
    ...
    `,typeof L=="function"){const u=m.querySelector(".weekly-card__rating");L(i,u)}m.addEventListener("click",()=>{C(o,()=>y(!0))}),v.appendChild(m)}),d+=A,d<a.length?h.style.display="block":h.style.display="none"}document.addEventListener("DOMContentLoaded",()=>{y(!0)});h.addEventListener("click",()=>{y()});const N=document.getElementById("genre-filter");N.addEventListener("change",()=>{const t=N.value,n=JSON.parse(localStorage.getItem(O))||[];if(t==="Genre"){y(!0);return}const a=n.filter(r=>Array.isArray(r.genres)&&r.genres.includes(t)?!0:Array.isArray(r.genre_ids)&&f?r.genre_ids.some(e=>f[e]===t):!1);d=0,v.innerHTML="",y(!1,a)});
//# sourceMappingURL=main-Bzr4Lkgk.js.map
